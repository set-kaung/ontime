...
func (prs *PostgresRequestService) InsertRequestReview(ctx context.Context, r Review) (int32, error) {
	tx, err := prs.DB.Begin(ctx)
	if err != nil {
	...
	if err := tx.Commit(ctx); err != nil {
		log.Println("InsertRequestReview: failed to commit transaction: ", err)
		return -1, internal.ErrInternalServerError
	}
